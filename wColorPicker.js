	...
	ColorPicker.prototype =
	{
		generate: function ()
		{
			...
			this.customOpacity = $('<input class="customOpacity" type="range" value="1" max="1" min="0" step="0.01"/>').on("input change", function(){
				...
				if(colorBox.indexOf('a') == -1){
   			 	colorBox = colorBox.replace(')', ', ' + $(this).val() + ')').replace('rgb', 'rgba');
				}
				
				else {
					var a = colorBox.slice(4).split(',');
					
					colorBox = 'rgba'+a[0]+','+parseInt(a[1])+','+parseInt(a[2])+',' + $(this).val() + ')';				
				}				
		...
		colorSelect: function($this, color)
		{
			...
			if ($this.customTarget.parents('._wPaint_strokeColorPicker').length) {
				...
				if(color.indexOf('a') == -1){
					...
					color = 'rgba'+a[0]+','+parseInt(a[1])+','+parseInt(a[2])+',' + opacityLevel + ')';				
				}				
